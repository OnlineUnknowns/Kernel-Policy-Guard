...
original = '''#define pr_fmt(fmt) KBUILD_MODNAME ": " fmt
...
MODULE_DESCRIPTION("Chronos dynamic hooking scaffold");
MODULE_VERSION("0.1");
...
new = Path('chronos_main.c').read_text(encoding='utf-8')

orig_lines = original.splitlines()
new_lines = new.splitlines()

# simple diff with unified header
result = []
for line in difflib.unified_diff(
    orig_lines,
    new_lines,
    fromfile='a/chronos_main.c',
    tofile='b/chronos_main.c',
    lineterm=''
):
    result.append(line)

Path('chronos_main.diff').write_text('\n'.join(result) + '\n', encoding='utf-8')
print('wrote chronos_main.diff')
